...
const CardsSection = ({ title, color, add }) => {
    ...
    return (
        <>
            <div ref={cardRef} className="w-1/5 bg-[#DEE6E8] h-full rounded-md p-3 flex flex-col items-center gap-14">
                <div className={`w-full relative ${isCollapsed ? "rotate-90" : "rotate-0"}`}>
                    <h3 ref={titleRef} className="bg-red-500 text-white py-2 px-4 rounded-md text-xs w-max absolute top-0 left-0" style={{ backgroundColor: color }}>0 {title}</h3>
                    ...
                    <div className="flex items-center gap-2 absolute top-0 right-0">
                        ...
                        <button ref={btnRef} className="bg-white p-1 rounded-md shadow-md" onClick={() => { setIsCollapsed(!isCollapsed) }}>
